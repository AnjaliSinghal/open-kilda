...
class FermaFlowMeterRepository extends FermaGenericRepository<FlowMeter, FlowMeterData, FlowMeterFrame>
    ...
    public MeterId findFirstUnassignedMeter(SwitchId switchId, MeterId startMeterId) {
        ...
        return framedGraph().traverse(g -> g.V().hasLabel(FlowMeterFrame.FRAME_LABEL)
                .has(FlowMeterFrame.METER_ID_PROPERTY, P.gte(MeterIdConverter.INSTANCE.toGraphProperty(startMeterId)))
    ...
    protected FlowMeterFrame doAdd(FlowMeterData data) {
        if ((Long) framedGraph().traverse(input -> input.V().hasLabel(FlowMeterFrame.FRAME_LABEL)
