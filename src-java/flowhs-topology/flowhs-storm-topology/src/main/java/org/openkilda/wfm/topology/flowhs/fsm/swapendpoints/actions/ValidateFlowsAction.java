...
public class ValidateFlowsAction
    ...
    protected void perform(State from, State to, Event event, FlowSwapEndpointsContext context,
        ...
        try {
            ...
            stateMachine.fireError(new ErrorData(e.getType(), e.getMessage(), FlowSwapEndpointsFsm.ERROR_DESCRIPTION));
            return;
        } catch (UnavailableFlowEndpointException e) {
            stateMachine.fireError(
                    new ErrorData(ErrorType.DATA_INVALID, e.getMessage(), FlowSwapEndpointsFsm.ERROR_DESCRIPTION));
            return;
        }

        try {
            persistenceManager.getTransactionManager().doInTransaction(() -> {
                String firstFlowId = stateMachine.getFirstFlowId();
                Flow foundFirstFlow = getFlow(firstFlowId);
                if (foundFirstFlow.getStatus() == FlowStatus.IN_PROGRESS) {
                    throw new FlowProcessingException(ErrorType.REQUEST_INVALID,
                            format("Flow %s is in progress now", firstFlowId));
                }

                String secondFlowId = stateMachine.getSecondFlowId();
                Flow foundSecondFlow = getFlow(secondFlowId);
                if (foundSecondFlow.getStatus() == FlowStatus.IN_PROGRESS) {
                    throw new FlowProcessingException(ErrorType.REQUEST_INVALID,
                            format("Flow %s is in progress now", secondFlowId));
                }
            ...
            stateMachine.fireError(new ErrorData(e.getErrorType(), e.getMessage(),
                    FlowSwapEndpointsFsm.ERROR_DESCRIPTION));
        }
