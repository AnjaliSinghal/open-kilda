...
public class RevertUpdateRequestAction
    ...
    public void execute(State from, State to, Event event, FlowSwapEndpointsContext context,
        ...
        stateMachine.setRequestCount(flowResponses.size());
        stateMachine.setAwaitingResponses(flowResponses.size());
        ...
        flowResponses.forEach(flowResponse -> {
            if (firstOriginalFlow.getFlowId().equals(flowResponse.getPayload().getFlowId())) {
                sendRevertUpdateCommand(firstOriginalFlow, secondOriginalFlow.getFlowId(), stateMachine);
            }

            if (secondOriginalFlow.getFlowId().equals(flowResponse.getPayload().getFlowId())) {
