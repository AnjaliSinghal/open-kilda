...
public class OnFinishedWithErrorAction
    ...
    protected Optional<Message> performWithResponse(State from, State to, Event event, FlowSwapEndpointsContext context,
        ...
        if (Event.TIMEOUT.equals(event)) {
            data = new ErrorData(ErrorType.OPERATION_TIMED_OUT, "Flow swap endpoints failed by timeout",
                    getGenericErrorMessage());
        } else if (Event.ERROR.equals(event)) {
            data = (ErrorData) context.getResponse();
        } else {
            List<String> flowIds = flowResponses.stream()
                    .map(FlowResponse::getPayload)
                    .map(FlowDto::getFlowId)
                    .collect(Collectors.toList());
            data = new ErrorData(ErrorType.REQUEST_INVALID,
                    format("Flow swap endpoints failed. Successfully reverted flows: %s", flowIds),
                    getGenericErrorMessage());
        }
        log.error("Swap endpoints operation failed. {}", data.getErrorMessage());
    ...
    protected String getGenericErrorMessage() {
        return FlowSwapEndpointsFsm.ERROR_DESCRIPTION;
    }
}
